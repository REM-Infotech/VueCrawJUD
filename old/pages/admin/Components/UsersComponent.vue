...
import { current_action, delete_call, form, submitForm } from "@shared/formusr";
...
onBeforeMount(async () => {
  try {
    ...
    items.value = response.data.database.map((item: formType) => {
...
function setupEdit(item: string[]) {
  form.id = parseInt(item[0]);
  form.name = item[1];
  form.login = item[2];
  form.email = item[3];
